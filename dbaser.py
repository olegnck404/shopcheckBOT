...
def edit_product(sku, quantity=None, price=None, photo=None):
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    if photo is not None:
        cursor.execute("UPDATE products SET photo = ? WHERE sku = ?", (photo, sku))
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            photo = input("Введите новый URL фото товара (или оставьте пустым для пропуска): ")

            quantity = int(quantity) if quantity else None
            price = float(price) if price else None

            edit_product(sku, quantity, price, photo)

        elif choice == '4':
            sku = input("Введите артикул товара (SKU) для удаления: ")
            delete_product(sku)
